...
//Calcola un indice che misura la vicinanza ai limiti di giunto per una configurazione di giunti data (più è alto, più si è vicini ai limiti)
inline double joint_limit_index(const Eigen::Vector<double, 7> &current_joint_config,
                  const Eigen::Vector<double, 7> &joint_min_limits =panda_constants::joint_min_limits,
                  const Eigen::Vector<double, 7> &joint_max_limits =panda_constants::joint_max_limits) {
  double index = 0;
  for (int i = 0; i < 7; i++) {
    index = index + 1 / (current_joint_config[i] - joint_min_limits[i]) + 1 / (joint_max_limits[i] - current_joint_config[i]);
  }
  return index;
}
//Calcola il gradiente dell'indice di vicinanza ai limiti di giunto per una configurazione di giunti data
inline Eigen::Vector<double, 7> joint_limit_index_grad(const Eigen::Vector<double, 7> &current_joint_config,
                       const Eigen::Vector<double, 7> &joint_min_limits = panda_constants::joint_min_limits,
                       const Eigen::Vector<double, 7> &joint_max_limits = panda_constants::joint_max_limits,
                       double step = 1e-3) {
  double index_0;
  ...
  // Calculating index 0
  index_0 = joint_limit_index(current_joint_config, joint_min_limits, joint_max_limits);
  ...
  for (int i = 0; i < 7; i++) {
    ...
    index_forwarded = joint_limit_index(forward_joint_config, joint_min_limits,joint_max_limits);
    limit_index_grad(i) = (index_forwarded - index_0) / step;
  }
  return limit_index_grad;
}

//Calcola l'indice di manipolabilità
inline double manip_index(const Eigen::Matrix<double, 6, 7> &jacob) {
  return std::sqrt((jacob * jacob.transpose()).determinant());
}

//Calcola il gradiente dell'indice di manipolabilità
inline Eigen::Vector<double, 7> manip_grad(const Eigen::Vector<double, 7> &current_joint_config,
    std::function<Eigen::Matrix<double, 6, 7>(const Eigen::Vector<double, 7> &)> jacob_calc,
    double step = 1e-3) {
...
inline double distance(geometry_msgs::msg::Point p1,geometry_msgs::msg::Point p2) {
...
std::array<double, 16> inline get_transform_matrix(const geometry_msgs::msg::Pose &pose_msg) {
...
inline geometry_msgs::msg::Pose get_pose(const std::array<double, 16> &transform_mat) {
...
inline Eigen::Matrix<double, 6, 7> get_jacobian(const std::array<double, 42> &jacob_raw) {
  Eigen::Map<const Eigen::Matrix<double, 6, 7>, Eigen::ColMajor> jacobian(jacob_raw.data());
  return jacobian;
}

inline geometry_msgs::msg::Pose convertMatrixToPose(const std::array<double, 16> &tf_matrix) {
...
struct decay_laws {
  ...
  decay_laws(double t_f, double q_i, double q_i_dot, double q_i_ddot,double lambda = 5, double beta = 10)
      : lambda(lambda), beta(beta), t_f(t_f), q_i(q_i), q_i_dot(q_i_dot), q_i_ddot(q_i_ddot) {
