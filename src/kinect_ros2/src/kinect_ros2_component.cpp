...
void KinectRosComponent::timer_callback()
{
  ...
  auto header_rgb = std_msgs::msg::Header();
  header.frame_id = "kinect_depth";
  header_rgb.frame_id = "kinect_rgb";

  auto stamp = now();
  header.stamp = stamp;
  depth_info_.header.stamp = stamp;

  header_rgb.stamp = stamp;
  ...
  if (_rgb_flag) {
    auto msg = cv_bridge::CvImage(header_rgb, "rgb8", _rgb_image).toImageMsg();
